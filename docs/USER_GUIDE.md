...
6. **テーブルデータの選択（オプション）**
   - アップロード後、テーブル一覧が表示されます
   - 各テーブルの推定行数（Rows approx）が表示されます
   - 除外したいテーブルのチェックを外すことで、そのテーブルのデータを除外できます
   - ⚠️ **FK制約の警告**: 除外するテーブルを参照している他のテーブルがある場合、警告が表示されます

> **💡 FK制約警告について**: 例えば `users` テーブルを除外すると、`orders` テーブルが `users` を参照している場合、`orders` のデータインポートも失敗する可能性があります。警告に表示される依存テーブルを確認し、必要に応じて一緒に除外してください。
...
4. **テーブルデータの選択**
   - アップロード後、比較ダンプのテーブル一覧が表示されます
   - 各テーブルの推定行数（Rows approx）が確認できます
   - 除外したいテーブルのチェックを外して、データを選択的に除外できます
   - ⚠️ **FK制約の警告**: 除外するテーブルを参照している他のテーブルがある場合、警告が表示されます

5. **「Restore & Compare」をクリック**
   - 選択したテーブルのみリストアされます
   - リストア → 解析 → 差分計算
